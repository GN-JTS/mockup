...
const EvaluatorDashboard = () => {
    ...
    const loadData = async () => {
      ...
      try {
        const [
          ...
          mockApi.getAppointmentRequestsByEvaluator(user.id),
          mockApi.getAppointmentsByUser(user.id),
          mockApi.getJobTitles(),
          mockApi.getGrades(),
        ]);

        // Filter employees based on evaluator scope AND their target promotion grade
        const employeesUnderEvaluation = allUsers.filter((u) => {
          ...
          // Check if evaluator can handle this employee's target grade
          return user.evaluatorFor?.some(
            (scope) =>
        ...
        const employeeIds = employeesUnderEvaluation.map((e) => e.id);
        ...
        setMyEmployees(employeesUnderEvaluation);
        ...
        console.error("Failed to load evaluator dashboard:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleApproveRequest = async (requestId: string) => {
    ...
    try {
      ...
      const requests = await mockApi.getAppointmentRequestsByEvaluator(user.id);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Perform final evaluations and certify employees
        </p>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {myEmployees.map((employee) => {
                ...
                const targetGrade = activePromotion
                  ...
                  : null;
                const { total, mastered, readyForEvaluation } =
                  getEmployeeEvaluatorProgress(employee.id);
                        ...
                        <div>
                          ...
                          {readyForEvaluation > 0 && (
                            <p className="text-xs text-green-600">
                              {readyForEvaluation} ready for evaluation
                            </p>
                          )}
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      {activePromotion ? (
                        <button
                          onClick={() =>
                            navigate(
                              `/evaluator/employee-promotion/${activePromotion.id}`
                            )
                          }
